...
#10
dim(EPAair_03_PM25_NC1819_Processed_Spread)
...
#13
dim(Up.Summary.EPAair_03_PM25_NC1819)
  
```

14. Why did we use the function `drop_na` rather than `na.omit`?

> Answer: We use the function `drop_na` to drop rows containing missing values in a particular column specified. If we were to use `na.omit` on our dataset, we would remove all NAs, including those in the PM2.5 and O3 columns, which we did not want to remove. Using `drop_na` allows us to only remove NAs in particular, specified columns. 
