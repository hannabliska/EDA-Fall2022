...
Plot_Temp <- ggplot(Peter.Paul.Nutrients.Chem, aes(
  x = fct_inorder(month.abb[month]), y = temperature_C)) +
...
Plot_TP <- ggplot(Peter.Paul.Nutrients.Chem, aes(
  x = fct_inorder(month.abb[month]), y = tp_ug)) +
...
Lake_Name_Legend <- get_legend(Plot_TP + mytheme + guides(color = guide_legend(nrow = 1))) #creating legend for cowplot
