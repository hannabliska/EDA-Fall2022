...
huc8_windenergy_updated <- read.csv(
...
joined_huc_wind <- merge(x=huc8_sf,
                         y=huc8_windenergy_updated,
                         by.x="HUC_8",
                         by.y="HUC_8")

#joined_huc_wind_sf <- st_as_sf(joined_huc_wind,)

#View the outputs
mapview(joined_huc_wind, zcol="AvgWindEnergy")

#View the outputs
ggplot(data=huc8_windenergy_updated) +
  geom_sf(aes(fill=AvgWindEnergy))
...
#Map the data
ggplot() + geom_sf(data=DWQ_Basin, aes(fill=ACRES))
