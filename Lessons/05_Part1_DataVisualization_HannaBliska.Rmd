...
library(ggridges) #helps build ridge plots (distribution of variable over time)
...
* color #change the color of contour
* fill #changing the color that is filling your bar or point #depending on plot, just one or the other
* shape #shape of the point you are using
* size #size of point you are using
* transparency #add transparency to points or bars

### Plotting continuous variables over time: Scatterplot and Line Plot

```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot #Fixing some things about the plot window (height 3, width 4)
...
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) #this adds a layer to the plot
  geom_point(aes(x = Date, y = Ozone), color = "blue") #outside provides blue color and no legend
print(O3plot2)
...
PMplot <- 
  ...
  geom_point() #different shape based on year #from same site will have the same color #one line per month
print(PMplot)
...
print(PMplot.faceted) #one per site, in three rows
...
print(PMplot.faceted2) #allows us to look at only three sites, compare 18 and 19
...
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line()
print(PMplot.line) #time series plot
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO) #two variables, one on x and one on y, to tell relationship between them

# Adjust axes to give a zoom here on particular area that we are interested in 
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #this allows us to put the deeper observations
print(tempvsdepth)

NvsP <- #two nutrients colored by depth (surface darker, lighter blue as down)
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm) + #linear line
  geom_abline(aes(slope = 16, intercept = 0)) #plot a line you know the slope and intercept for
print(NvsP)
#hanna question: can you change the color scale so that its darker deeper?
...
Better than barcharts with error bars. Generate error bars for data set with box plots and violin plots.

```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# Box and whiskers plot
Nutrientplot3 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? color regards contour of shape, fill will change the fill of the boxplot
print(Nutrientplot3)

#boxplot is helpful for distribution of data, smaller amount of data

# Dot plot
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, 
               stackdir = "center", position = "dodge", dotsize = 2) #stacking on center, on y, 
print(Nutrientplot4)
...
#also helpful for distribution, more data
#similar to box plots (but box only shows summary stats) but violin also shows density
...
print(Nutrientplot7)
#much better practice with the gathered dataset

# Frequency polygons have the risk of becoming spaghetti plots. 
#Hard to read with more than 5 groups
