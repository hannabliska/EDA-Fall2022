...
#View using ggplot #plots geographic space
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), #x and y coordinate columns
           #longitude is x whereas latitude is y
           crs=4326)

#View the data using the methods above...
class(EPAair.sf)

#Viewing EPA sites using plot from `sf`
plot(EPAair.sf['mean_Ozone'])

#Viewing EPA sites using ggplot
ggplot(data=EPAair.sf) + geom_sf()

#Viewing EPA sites using mapview
mapview(EPAair.sf)
```
...
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #crs telling lat long
class(sfc_GraingerHall)
mapview(sfc_GraingerHall) #now see correct location
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) #takes original and transforms
#to use utm
...
#EPAair.sf.utm  <-  EPAair.sf.utm %>% 
# mutate(mean.distance=mean_distances)

#ggplot with different size based on the average distance
ggplot(EPAair.sf.utm, aes(x=COUNTY, y=mean_Ozone)) +
