...
Null -> mean of all populations are equal
Alternative -> not all of the means are equal

Factor -> quantity under examination in an experiment
Levels -> categories, measurements, or strata of the factor of interest (also called populations)
Balanced design -> an experiment has a balanced design if the factor levels have equal sample sizes

Assumes: all pops are normally distributed, variances are equal, observations are independent
...
Litter.Totals <- Litter %>%
  ...
  summarise(dryMass = sum(dryMass)) #subset of litter with only the info we want to explore
