...
huc8_sf_Upper <- st_read('./Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN == "Upper Neuse")


#Create a map
mapview(huc8_sf_Upper)
...
Joining data to spatial features works the same as joining tables: we just need a common attribute to link the two datasets. Here, we'll add demographic data to our Census county feature, using the State&County FIPS code as the common attributes. #looks for common attribute
...
counties_sf_join <-  merge(x = counties_sf,
                           ...
                           by.x = "GEOID", 
                           by.y = "FIPS" )
#specify the two columns we are usign to join them together
...
huc8_wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  ...
  mutate(HUC_8=substr(HUC_12,1,8)) %>% #specify a string
  #grab from the 1st to the 8th value within HUC_12
  group_by(HUC_8) %>%
  summarize(kWhkm2day=sum(kWhkm2day))

#Join to HUC_8 features
huc8_wind_sf_join <- merge(x=huc8_sf,
                           y=huc8_wind,
                           by.x="HUC_8",
                           by.y="HUC_8")

#View the outputs
mapview(huc8_wind_sf_join, zcol="kWhkm2day")
...
ggplot() +
  geom_sf(data=agg_huc8,aes(fill=ACRES))

mapview(agg_huc8, zcol="DWQ_Basin")
...
the_triangle <- counties_sf_utm %>%
  filter(NAME=="Durham" | NAME=="Chatham" | NAME=="Orange" | NAME=="Wake")

  
#Grab the intersecting HUC_8s
the_hucs <- huc8_sf_utm[the_triangle,]
#what you are intersecting #with what


#Intersect the HUC_8s NOT CORRECT! #ask in class
hucs_within <- the_triangle %>% #cookie cutter
  st_intersection(huc8_sf_utm) #cookie dough

hucs_within <- huc2_utm %>%
...
result_Mask30km <- st_intersects(counties_sf_utm,
                                 site_30km,
                                 sparse=FALSE)    

selCounties_30km <-counties_sf_utm[result_Mask30km,]
