...
#
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean), bins=15) + scale_x_continuous(limits=c(0,6))
...
> Answer: 
Bar plot would be most useful for caetgorical data. For example, we used bar plots here to learn about the number of records of mean discharge that were "approved." Histograms are most useful for looking at continuous data, because they allow us to see patterns using bins. I would use frequency line polygons to look at the distribution of frequencies. Box-and-whisker allow us to also look at distributions. Scatterplots help us to look at relationships among variables (not within, like the others).

## Ending discussion

What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: I was able to learn about the different patterns within discharge and gauge height. I was also able to discern patterns regarding categorical measures (approval, for example) within these data sets. For example, I could see that there was a range of gauge height means that were approved.


How can multiple options for data exploration inform our understanding of our data?

> Answer: This allows us to understand patterns within variables (is there a central tendency for the discharge mean?) as well as among variables (how does the discharge mean relate to the gauge height mean?)
...
> Answer: We discussed how gauge height does not increase at the same rate as discharge because of the bowl-shape of rivers. I also noticed that gauge height centers around 2 but the discharge data is skewed toward zero; with some exceptions for flood days.
