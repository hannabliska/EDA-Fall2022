...
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #this is needed
summary(NTL.nutrient.data.PeterPaul$lakename) #only Paul Lake and Peter Lake
...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #selected data frame
  mutate(month = month(sampledate)) %>% #month
  select(lakeid:daynum, month, sampledate:comments) %>% #reorder
  drop_na(depth) #get rid of NAs
...
NTL.phys.data.PeterPaul.subset <- select(NTL.phys.data.PeterPaul, 
                                         lakename:irradianceDeck) #removed two variables
  
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul, 
                                             lakename, year4, daynum, month, sampledate, depth:po4) #removed 3 variables, lake ID, month, and lake ID
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #four columns into two, one with category and other with values

NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #removed NAs for concentration
...
NTL.nutrient.data.PeterPaul.spread2 <- pivot_wider(NTL.nutrient.data.PeterPaul.gathered, names_from = nutrient, values_from = concentration) #names from nutrient, values from concentration
...
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset) #joining by lakename, year 4, daynum, month, sampledate, and depth. Six common columns, but four from physical (oxygen, light, temp etc) and five nutrients from the nutrients.
...
The Niwot Ridge litter dataset, when downloaded from NEON, comes packaged with each month as a different .csv file. If we want to analyze the dataset as a single data frame, we need to combine each of these files. #Binding datasets by rows. Sometimes you will find a cvs per month or year. Important to combine into one. We could have one line of code for each cvs files
...
```{r}
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE) #list all the files listed in this folder
LitterFiles #copied to object the file name for all 20 files we need to read

Litter <- LitterFiles %>%
  plyr::ldply(read.csv) #we can use a pipe (specify an order of functions to perform) and we are going to, for litter files, read each element on that file
```
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID")) #common names, joining by those conditions
...
LitterTrap <- LitterTrap %>%
  select(plotID:trapID, collectDate, functionalGroup:qaDryMass, subplotID:geodeticDatum) #only treating the columns, only have 13 variables now
...
NTL.PeterPaul.summaries <- 
  NTL.phys.nutrient.data.PeterPaul %>% #dataset
  filter(depth == 0) %>% #just want data on surface
  group_by(lakename, month) %>% #want to group by lake name and by month
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #filter the NAs, none of these three are NAs
  summarise(meantemp = mean(temperature_C), #summaries to create
            sdtemp = sd(temperature_C), 
            meanTN = mean(tn_ug), 
            sdTN = sd(tn_ug), 
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug))
#inspect the summaries object
