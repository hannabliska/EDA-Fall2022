...
## Announcements
1. Last assignment
2. No class next week
3. Work on class project after turning in last assignment
4. Class presentation on project on Wednesday after Thanksgiving
...
#Set the URL
theURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02087500&period=7&format=rdb' #saving web addressed (stripped) as a string
...
new_gage_data <- read.table(newURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T)

colnames(new_gage_data) = c(
  "agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

new_gage_data <- new_gage_data %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

ggplot(new_gage_data,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
...
#2019 Repeat
#1 Link to the web site using read_html
the_website_2019 <- read_html("https://www.eia.gov/electricity/state/archive/2019/")

#2&3 Locate elements and read their text attributes into variables
the_states_2019 <- the_website_2019 %>%
  html_nodes("td:nth-child(1)") %>%
  html_text()

the_price_2019 <- the_website_2019 %>%
  html_nodes("td:nth-child(2)") %>%
  html_text()
  
the_capacity_2019 <- the_website_2019 %>%
  html_nodes("td:nth-child(3)") %>%
  html_text()

net_generation_2019 <- the_website_2019 %>%
  html_nodes("td:nth-child(4)") %>%
  html_text()
  
total_retail_2019 <- the_website_2019 %>%
  html_nodes(" td:nth-child(5)") %>%
  html_text()
  
#3 Construct a dataframe from the values

US_Electricity_2019 <- data.frame(
  "States"=the_states_2019,
  "Price"=the_price_2019,
  "Capacity"=the_capacity_2019,
  "Net Generation"=net_generation_2019,
  "Total Retail"=total_retail_2019
)
```
...
the_url <- ifelse(the_year==2021,
                    'https://www.eia.gov/electricity/state/',
                    paste0(
                      'https://www.eia.gov/electricity/state/archive/',the_year,'/'))
...
df.2017 <- scrape.it(the_year=2017)
