...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))) #want text to have Greek letters or an equation #paste things together (common characters under quotes)
print(Plot_text)
...
SurfacePeterPaul <- filter(PeterPaul.chem.nutrients, depth==0)

Plot01 <- ggplot(SurfacePeterPaul, aes(x=daynum, y=temperature_C)) +
...
Plot02 <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate, y=temperature_C)) +
...
Plot03 <-ggplot(EPAair, aes(x=PM2.5, y=Ozone)) +
  geom_point(aes(color=meanlat), size=0.5, alpha=0.5) +
...
Plot04 <- ggplot(EPAair, aes(x=month, y=PM2.5, color=year)) +
  geom_point()
print(Plot04)

Plot05 <- ggplot(EPAair, aes(x=as.factor(year), y=PM2.5)) +
  geom_violin()
print(Plot05)
```
