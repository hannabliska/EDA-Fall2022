...
### Data scraping notes
NGOs and govt will be providing data online. Some have API - in which you are able to link through R or Python and send instructions to the machine to download it. Will look to see if the data are there, but if there's no download link or easy API registered with the website. Could paste it - but that will be cumbersome. Leaves us with our tools to do our scraping programmatically. Will learn a bit more about datascraping as we do the exercise. Ethical considerations about scraping data: not all are designed to be scraped. Look for terms of service. If they say the data are reserved for internal use, you shouldn't scrape. Some websites have a file called robots.txt: guard against scraping by web bots. First look to see if you can directly download the data. Second, look for an API (programmatically access data). Census has an API! So you can use R to pull the data off. Third, want to look and see if you allow scraping via the terms of service or robots.txt. Also consider: web pages might be built for viewing. SO you can put pauses in so that you don't overwhelm the website.
