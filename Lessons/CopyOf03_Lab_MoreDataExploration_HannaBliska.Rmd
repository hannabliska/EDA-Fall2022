...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # overriding column names with own names (same as col names)

dataframe1[1:2,] # slices
dataframe1[c("ID","Passed")] # refer to them by their column names
dataframe1$ID
...
> Answer:List of the column names.
...
```{r DataExercise-1a}
high_temp <- c(93, 73, 78, 85, 82, 78, 75, 73, 71, 76)
low_temp <- c(53, 48, 59, 61, 54, 51, 51, 52, 55, 55)
...
```{r DataExercise-1b}
celcius_conversion <- function(high_temp) {(high_temp-32)*(5/9)}
high_temp_celcius <- celcius_conversion(high_temp)

celcius_conversion <- function(low_temp) {(low_temp-32)*(5/9)}
low_temp_celcius <- celcius_conversion(low_temp)
...
```{r DataExercise-1c}
data.frame(high_temp, high_temp_celcius, low_temp, low_temp_celcius)
temperature_df <- data.frame(high_temp, high_temp_celcius, low_temp, low_temp_celcius)
temperature_df

names(temperature_df) <- c("High F", "High C", "Low F", "Low C")
```
