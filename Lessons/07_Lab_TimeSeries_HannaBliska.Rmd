...
#Hanna Class Notes#
#Indexed by time, worried about when each observation was collected
#If indexed by time, look at season and trend of data
#Can observe trends by month, days
#We also always have a random component
#Models assume stationarity of the series, but in reality, we don;t have stationary time series. Seen change in behavior. Important to perform tasks to see if we see an increase (trend) over time or has the temperature remained the same throughout time (stationary).
...
Temp_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% 
  mutate( Date = my(paste0(Month,"-",Year))) %>% 
  select(Date,Temperature_C) #when we don't specify the day
#it just takes the first day for each month
...
#Exercise 6: Plot both series over time on separate plots. Add a trend line using geom_smooth(). Can you say something about the trend just by visualizing the data? #Trend increasing in temperature, looks stable in rain
...
#matches expectation that temperatures are increasing
...
Plot.Rain <-
  ...
  geom_smooth(method=lm, se = FALSE, color="blue")
print(Plot.Rain)
#no slope on the linear trend, no trend
#can see the up and down movements -> seasonal trend
...
temp.ts <- ts(BRA_complete$Temperature_C, start=c(1901,1), frequency=12)
...
ts.rainfall.decomposed <- stl(rainfall.ts, s.window="periodic")
plot(ts.rainfall.decomposed)

ts.temp.decomposed <- stl(temp.ts, s.window="periodic")
plot(ts.temp.decomposed)
#temperature has much more even bars
...
ts.rainfall.components <- as.data.frame(ts.rainfall.decomposed$time.series[,1:3])

ts.rainfall.components <-
  ts.rainfall.components %>%
  mutate(date=BRA_complete$Date) %>%
  mutate(observed=BRA_complete$Rainfall)

ts.temp.components <-
  as.data.frame(ts.temp.decomposed$time.series[,1:3])

ts.temp.components <-
  ts.temp.components %>%
  mutate(date=BRA_complete$Date) %>% #add date
  mutate(observed=BRA_complete$Temperature_C) #add observed

# Exercise 10: Visualize how the trend maps onto the data for both series
#observed = trend + seasonality

Trend.Plot.Temp <- ggplot(ts.temp.components) +
  geom_line(aes(x = date, y = observed)) +
  geom_line(aes(x = date, y = trend), color="red") +
  ylab("Temperature")
print(Trend.Plot.Temp)

Trend.Plot.Rain <- ggplot(ts.rainfall.components) +
  geom_line(aes(x = date, y = observed)) +
  geom_line(aes(x = date, y = trend), color="red") +
  ylab("Rain")
print(Trend.Plot.Rain)
